// ── Vega SOP Template ──────────────────────────────────────
// Based on Vega_SOP_Generation_Template.docx
// This is the canonical template for all Vega SOPs.

// Bump this when the template structure changes.
// SOPs created on older versions will be flagged for update during review.
...
/**
 * Generate SOP HTML from the canonical template.
 */
export function generateSopHtml({ sopId, title, scope, category, businessUnit, owner, author, checker, reviewCadence, date }) {
...
<div class="title-block">
  ...
  <div class="sop-scope">${scope || ''}</div>
</div>

<div class="meta-block">
  <p><strong>SOP ID:</strong> ${sopId || 'SOP-001'}</p>
  <p><strong>Business Unit:</strong> ${businessUnit || 'TBD'}</p>
...
<div class="sub-label">Purpose</div>
<p></p>

<div class="sub-label">Trigger</div>
<p></p>

<div class="sub-label">Successful Completion</div>
<p></p>

<div class="sub-label">Key Systems and Tools</div>
<table>
  <thead><tr><th>System / Tool</th><th>Used For</th><th>Who Has Access</th></tr></thead>
  <tbody>
    <tr><td></td><td></td><td></td></tr>
  </tbody>
</table>

<h2>2. Procedure Steps</h2>
...
<table>
  <thead><tr><th style="width:55px;">Step</th><th>Action</th><th style="width:80px;">Role</th><th style="width:100px;">System</th></tr></thead>
  <tbody>
    <tr>
      <td><span class="step-num">01</span></td>
      <td><span class="step-action"></span><span class="step-detail"></span></td>
...
<table>
  <thead><tr><th>Confirm Before Closing Out</th><th style="width:100px;">Verified By</th></tr></thead>
  <tbody>
...
<table>
  <thead><tr><th>Party / Role</th><th>Name</th><th>Email or Contact</th></tr></thead>
  <tbody>
    <tr><td></td><td></td><td></td></tr>
  </tbody>
</table>

<h2>5. Exceptions and Edge Cases</h2>
<p></p>

<h2>6. Review Schedule</h2>
<p></p>
...
// CSS classes are stripped. This function re-wraps it.

const SOP_ID_RE = /^[A-Z]{2,5}-[A-Z]{2,5}-\d{3}$/
const META_LABEL_RE = /^(SOP ID|Business Unit|Version|Effective|Category|Owner|Maker|Checker|Review Cycle|Classification)\s*:/i
const HEADER_LINES = [
...
function enhanceElement(el) {
  ...
  if (tag === 'H2') return `<h2>${el.innerHTML}</h2>\n`
  if (tag === 'H3') return `<h3>${el.innerHTML}</h3>\n`
  ...
  // Detect sub-labels by known names
  if (tag === 'P' && ['purpose', 'trigger', 'successful completion', 'key systems and tools', 'key systems'].includes(text.toLowerCase())) {
    return `<div class="sub-label">${text}</div>\n`
  }

  if (tag === 'TABLE') return enhanceTable(el)

  if (tag === 'UL' || tag === 'OL') return el.outerHTML + '\n'

  if (tag === 'P') return `<p>${el.innerHTML}</p>\n`
  ...
  rows.forEach((row, rowIdx) => {
    ...
    cells.forEach((cell, colIdx) => {
      ...
      if (isHeader && isStepTable) {
        ...
        if (headers[colIdx] === 'system') attrs = ' style="width:100px;"'
      }
...
// ── Strip unfilled sections from published view ──────────────
...
export function stripUnfilledSections(html) {
  ...
  for (const h2 of headings) {
    ...
    if (headingText.includes('revision history')) continue
    ...
    // Remove if empty or only AI placeholders
    if (!sectionText || PLACEHOLDER_SIGNATURES.some(sig => sectionText.includes(sig))) {
      // Check if tables have real content
      const tables = sectionEls.filter(e => e.tagName === 'TABLE')
      const allTablesEmpty = tables.every(table => {
      ...
      if (tables.length === 0 || allTablesEmpty) {
